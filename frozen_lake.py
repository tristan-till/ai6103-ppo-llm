...
def generate_random_map(
    ...
    while not valid:
        ...
        board = ["".join(flat_board[i:i+len(board[0])]) for i in range(0, len(flat_board), len(board[0]))]
...
class FrozenLakeEnv(Env):
    ...
    * v0: Initial version release
    ...
    def __init__(
        ...
        self.P = [
            {s: {a: [] for a in range(nA)} for s in range(nS)},  # Transition matrix for Agent 1
            {s: {a: [] for a in range(nA)} for s in range(nS)},  # Transition matrix for Agent 2
        ]
    ...
    def step(self, action, agent_id):
        """
        Step function to handle transitions for a single agent.
        `action` is the action taken by the specified agent.
        `agent_id` specifies which agent's state to update.
        """
        # Get the transition probabilities for the current state and action
        transitions = self.P[self.s[agent_id]][action]

        # Sample the next state based on the probabilities
        i = categorical_sample([t[0] for t in transitions], self.np_random)
        p, s, r, t = transitions[i]

        # Update the agent's state and last action
        self.s[agent_id] = s
        self.lastaction[agent_id] = action
    ...
    def reset(
        ...
        *,
        ...
        self.s = [
            categorical_sample(self.initial_state_distrib, self.np_random),
            categorical_sample(self.initial_state_distrib, self.np_random),
        ]
        ...
        if self.render_mode == "human":
            self.render()
        return [int(self.s[0]), int(self.s[1])], {"prob": [1, 1]}
